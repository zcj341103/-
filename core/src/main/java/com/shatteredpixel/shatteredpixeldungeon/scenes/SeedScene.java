...
public class SeedScene extends PixelScene {
	...
	public void create() {
		...
		//将之前记录的数据 从新给到它 退出界面重进 省再写一遍
		textBox.setText((sf.input == null || sf.input.equals("")) ? "":sf.input);
		...
		StyledButton introButton = new StyledButton(Chrome.Type.GREY_BUTTON_TR,"介绍"){
			...
			public void onClick() {
				...
				zw.text("1、随机种子按钮：随机一个种子的前4层物品信息。不包括怪物掉落的物品。\n2、输入框：输入可查范围内xx之戒|法杖|神器,每个物品中用\" \"空格隔开。回车确定，再点随机种子按钮查看。不支持查等级。最大查次数1000次。\n3、点我设查的种子：可输入查看的种子。\n4、设置先挑战，查找的都是挑战种子。\n5、注意死亡继承物品因素会导致种子内容变化。");
	...
	protected void onFind(Component content, float fullWidth, boolean landscape){
		...
		String[] data = sf.addDungeon(30,1000, seed_, sf.getData(sf.input));
